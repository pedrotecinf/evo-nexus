...
interface FloatingChatWindowProps {
  window: FloatWindow
  /** Stack index (0 = rightmost). Used to offset position. */
  index: number
}
...
export default function FloatingChatWindow({ window: win, index }: FloatingChatWindowProps) {
  const { closeWindow, toggleMinimize, toggleViewMode, updateApprovals } = useFloatingChat()
  const meta = getAgentMeta(win.id)
  const accentColor = meta.color || '#00FFA7'

  // Horizontal position: stack windows to the left, each offset by (width + gap)
  const right = GAP + index * (WINDOW_WIDTH + GAP)
