...
describe('MentionAutocomplete', () => {
  it('mostra opções selecionáveis e omite locked', () => {
    ...
    render(<MentionAutocomplete agents={[...agents, { name: 'vault', description: 'locked', locked: true }]} error={false} context={context} assignee={null} selectedIndex={0} onSelect={onSelect} onSelectedIndexChange={vi.fn()} anchorRef={{ current: null }} listboxId="mentions" />)
  ...
  it('calcula direção e altura em viewport pequena', () => {
    expect(mentionMenuGeometry({ top: 260, bottom: 480 } as DOMRect, 500)).toEqual({ direction: 'above', maxHeight: 224 })
    expect(mentionMenuGeometry({ top: 20, bottom: 100 } as DOMRect, 500)).toEqual({ direction: 'below', maxHeight: 224 })
